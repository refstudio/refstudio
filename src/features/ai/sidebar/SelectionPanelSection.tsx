...
export function SelectionPanelSection() {
  ...
  return (
    <PanelSection title="Selection">
      ...
      {selection && <RewriteWidget className="mb-6" selection={debouncedSelection} />}
    </PanelSection>
  );
}
...
export function RewriteWidget({
  ...
  const {
    ...
    queryKey: [selection, rewriteOptions], // clear rewrite when the selection changes
    enabled: false,
    ...
    <div className={cx('flex flex-col gap-4 px-4', className)}>
      ...
      {rewrite && (
        <div className="flex flex-col rounded-xl rounded-t-none border-primary bg-primary/50">
          ...
          <button className="btn-primary rounded-t-none" onClick={() => handleSelectChoice(selectedChoice)}>
            REPLACE
          </button>
        </div>
      )}
